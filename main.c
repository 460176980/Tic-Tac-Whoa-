...
int color=0; //color=1上色(O蓝，X红)，0默认白色
    ...
    for (j = 0; j < N; j++) {
        ...
        for (i = 0; i < N; i++) {
            ...
            switch (s->board[j][i]) {
                ...
                case  0: if(color==1) printf("\033[44m O \033[0m");else printf(" O ");break;
                case  1: if(color==1) printf("\033[31m X \033[0m");else printf(" X ");break;
            }
            if (i < N-1 )
                printf("|");
            else
...
void Setting(){
		...
		printf("3. whether to change color(X red,O blue),input Y\n");//根据数字选定颜色
		char c;
		getchar(); 
		scanf("%c",&c);
		if(c=='N'){
...
int main(){
	...
	while(1){
		...
		switch(choose){
			...
			case 4: return 0;
			default: printf("option is error\n"); 
		}
	}
